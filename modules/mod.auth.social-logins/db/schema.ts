...
export const modAuthSocialOauthStates = pgTable(
  'mod_auth_social_oauth_states',
  {
    ...
    stateNonce: varchar('state_nonce', { length: 180 }).notNull(),
  ...
  (table) => ({
    stateTokenUnique: uniqueIndex('mod_auth_social_oauth_state_token_idx').on(
      table.stateToken
    ),
    providerExpiresIndex: index('mod_auth_social_oauth_provider_exp_idx').on(
      table.providerId,
      table.expiresAt
    ),
    requestedByIndex: index('mod_auth_social_oauth_requested_by_idx').on(
      table.requestedByUserId
    ),
    flowCheck: check(
      'mod_auth_social_oauth_flow_chk',
      sql`${table.flow} in ('login', 'link')`
    ),
    areaCheck: check(
      'mod_auth_social_oauth_area_chk',
      sql`${table.area} in ('admin', 'dashboard')`
    )
  })
);

export type ModAuthSocialOauthState = typeof modAuthSocialOauthStates.$inferSelect;
export type NewModAuthSocialOauthState =
  typeof modAuthSocialOauthStates.$inferInsert;
