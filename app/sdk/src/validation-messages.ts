...
export type BuildFormValidationMessageValue =
  | string
  | number
  | boolean
  | null
...
export type BuildFormValidationMessageValues = Record<
  string,
...
export type BuildFormValidationMessageInput =
  | string
  | BuildFormValidationMessageDescriptor;

export type BuildFormValidationMessageResolver = (
  descriptor: BuildFormValidationMessageDescriptor
...
function normalizeMessageTemplate(value: string) {
  return value.trim();
}

function interpolateMessageTemplate(
  template: string,
  values: BuildFormValidationMessageValues | undefined
) {
  if (!values) {
    return template;
  }
...
function normalizePositiveIntValue(value: unknown) {
...
export function createBuildFormValidationMessage(
  key: string,
  fallback: string,
  values?: BuildFormValidationMessageValues
): BuildFormValidationMessageDescriptor {
...
export function formatBuildFormValidationMessage(
  template: string,
  values?: BuildFormValidationMessageValues
) {
...
export function resolveBuildFormValidationMessage(
  input: BuildFormValidationMessageInput,
  resolver?: BuildFormValidationMessageResolver
) {
...
export function createCatalogBuildFormValidationMessageResolver(
  catalog: BuildFormValidationMessageCatalog
...
export const buildFormValidationMessage = {
  required(label = 'This field') {
    return createBuildFormValidationMessage(
      'build_form.validation.required',
      '{label} is required.',
      { label }
    );
  },
  ...
  invalidSelection(label = 'Selection') {
    return createBuildFormValidationMessage(
      'build_form.validation.invalid_selection',
      'Selected {label} is invalid.',
      { label }
    );
  },
  recordNotFound(label = 'Record') {
    return createBuildFormValidationMessage(
      'build_form.validation.record_not_found',
      '{label} was not found.',
      { label }
    );
  },
  alreadyExists(label = 'Value') {
    return createBuildFormValidationMessage(
      'build_form.validation.already_exists',
