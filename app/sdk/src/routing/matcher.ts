 ...
 */
export function resolveAreaFallbackChain(pathname: string): RouteProxyFn[] {
 ...
 */
export function matchRouteProxyChain(pathname: string): RouteProxyFn[] {
  const routes = getAllRegisteredRoutes();

  let bestMatch: { path: string; proxies: RouteProxyFn[] } | null = null;
