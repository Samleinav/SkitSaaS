...
function resolveQueryOptionDefaults(options: BuildTableQueryParamOptions = {}) {
  return {
...
function toUrlSearchParams(input: URLSearchParams | string | undefined) {
...
export function parseBuildTableQueryState(
  input: URLSearchParams | string,
  options: BuildTableQueryParamOptions = {}
): BuildTableQueryState {
  const params = toUrlSearchParams(input);
  const keys = resolveQueryOptionDefaults(options);
  const filters = Array.from(params.entries()).reduce<BuildTableQueryFilters>(
...
export function createBuildTableQuerySearchParams(
