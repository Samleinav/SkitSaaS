...
function readItemValue<TItem extends Record<string, unknown>>(
  item: TItem,
  key: keyof TItem | string
) {
  if (typeof key !== 'string') {
    return item[key] as BuildTableValue;
  }

  return item[key as keyof TItem] as BuildTableValue;
}
...
function resolveSortValue<TItem extends Record<string, unknown>>({
  item,
  column
}: {
  item: TItem;
...
function compareBuildTableValues(left: BuildTableValue, right: BuildTableValue) {
...
function findColumnById<TItem extends Record<string, unknown>>(
  definition: BuildTableDefinition<TItem>,
  columnId: string
) {
  return (
    definition.columns.find((column) => String(column.key) === columnId) ?? null
  );
}

function resolveFilterValue<TItem extends Record<string, unknown>>({
  item,
  filter,
  definition
}: {
  item: TItem;
  filter: BuildTableFilterDefinition<TItem>;
  definition: BuildTableDefinition<TItem>;
}) {
  if (filter.getValue) {
...
function resolveDefaultFilters<TItem extends Record<string, unknown>>(
  definition: BuildTableDefinition<TItem>
) {
  const filters = definition.toolbar?.filters ?? [];
  return filters.reduce<BuildTableQueryFilters>((accumulator, filter) => {
...
function resolvePageSize(
  pagination: BuildTablePaginationDefinition | undefined,
  query: BuildTableQueryState | undefined
) {
  return normalizeBuildTablePageSize(
    query?.pageSize,
    pagination?.pageSize ?? DEFAULT_BUILD_TABLE_PAGE_SIZE
  );
}

export function normalizeBuildTablePage(value: unknown, fallback = 1) {
  const parsed =
    typeof value === 'number' ? value : Number.parseInt(String(value ?? ''), 10);
  return Number.isInteger(parsed) && parsed > 0 ? parsed : fallback;
}

export function normalizeBuildTablePageSize(value: unknown, fallback = 10) {
  const parsed =
    typeof value === 'number' ? value : Number.parseInt(String(value ?? ''), 10);
  return Number.isInteger(parsed) && parsed > 0 ? parsed : fallback;
}

export function normalizeBuildTableSortDirection(
  value: unknown
): BuildTableSortDirection | null {
  return value === 'asc' || value === 'desc' ? value : null;
}

export function normalizeBuildTableQueryState<TItem extends Record<string, unknown>>(
  query: BuildTableQueryState | undefined,
  definition?: BuildTableDefinition<TItem>
): BuildTableQueryState {
  ...
  return {
    ...
    page: normalizeBuildTablePage(
      query?.page,
      definition?.query?.page ?? DEFAULT_BUILD_TABLE_PAGE
    ),
    pageSize
  };
}

export function filterBuildTableData<TItem extends Record<string, unknown>>(
  definition: BuildTableDefinition<TItem>,
  query: BuildTableQueryState | undefined
) {
  ...
  const searchColumns =
    definition.toolbar?.search?.columns?.map((column) => String(column)) ?? null;
  ...
  return definition.data.filter((item) => {
    ...
    return toolbarFilters.every((filter) => {
      ...
      const itemValue = normalizeSearchValue(
        resolveFilterValue({
          item,
          filter,
          definition
...
export function sortBuildTableData<TItem extends Record<string, unknown>>(
  definition: BuildTableDefinition<TItem>,
  items: TItem[],
  sorting: BuildTableSortingState | null | undefined
) {
  if (!sorting) {
    return items;
  }
...
export function paginateBuildTableData<TItem extends Record<string, unknown>>({
  items,
  page,
  pageSize
}: {
...
export function formatBuildTablePaginationSummary({
  startRow,
  endRow,
  totalItems,
...
export function resolveBuildTableView<TItem extends Record<string, unknown>>(
  definition: BuildTableDefinition<TItem>,
  query: BuildTableQueryState | undefined = definition.query
): ResolvedBuildTableView<TItem> {
  ...
  const sortedItems = sortBuildTableData(
    definition,
    filteredItems,
