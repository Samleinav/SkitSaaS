...
| Portal API under `/api/<portalName>/*` | `RouteApiPortal(...)` in `src/routes.ts` plus host bridge files under `app/api/<portalName>/*/route.ts` using `withApiRouteEntries(...)` around `RouteApiPortal(...).handler(...)` entries | `src/routes.ts`, `app/api/<portalName>/*/route.ts`, `lib/routing/with-api-route.ts` | Do not force portal APIs through `/api/modules/*` when the actual requirement is a portal-scoped endpoint. |
