...
type ModuleJson = {
  ...
  sdkRange?: string;
  /**
   * Path (relative to module dir) to the edge-safe routes.ts file.
   * When set, modules:prepare adds an import to lib/routing/all-routes.generated.ts
   * so the middleware proxy chain is registered automatically.
   */
  routesEntry?: string;
  /**
   * Path (relative to module dir) to the portal-init.ts file.
   * When set, modules:prepare adds an import to lib/portals/all-portals.generated.ts
   * so portal pages are registered in the Node.js server context automatically.
   */
  portalInit?: string;
  templatePack?: {
    defaultEntry?: string;
...
export type ResolvedModule = {
  ...
  portalInitImportPath: string | null;
};
...
export type ModulesPrepareResult = {
  ...
  strictCompatibility: boolean;
  outputPath: string;
  allRoutesGeneratedPath: string;
  allPortalsGeneratedPath: string;
  warnings: string[];
  configErrors: ModulePrepareConfigError[];
...
function resolveOptionalModuleFile(
  ...
  const absolute = path.isAbsolute(value.trim())
    ? value.trim()
    : path.join(moduleDir, value.trim());
  if (!fs.existsSync(absolute)) {
    warnings.push(
      `Module ${moduleId} declares ${field}="${value}" but the file does not exist.`
    );
    return null;
  }
  return resolveImportPath(rootDir, absolute);
}
  ...
  if (modulesDir && fs.existsSync(modulesDir)) {
    ...
    for (const moduleDir of moduleDirs) {
      ...
      resolvedModules.push({
        ...
        routesImportPath: resolveOptionalModuleFile(
          rootDir,
          moduleDir,
          moduleJson.moduleId,
          'routesEntry',
          moduleJson.routesEntry,
          warnings
        ),
        portalInitImportPath: resolveOptionalModuleFile(
          rootDir,
          moduleDir,
          moduleJson.moduleId,
          'portalInit',
          moduleJson.portalInit,
          warnings
        ),
      });
    }
...
`;
  ...
  // Generate all-routes.generated.ts — side-effect imports for module routes
  const allRoutesGeneratedPath = path.join(rootDir, 'lib', 'routing', 'all-routes.generated.ts');
  const routesImports = resolvedModules
    .filter((mod) => mod.routesImportPath)
    .map((mod) => `import '${mod.routesImportPath}';`)
    .join('\n');
  const allRoutesGeneratedBody =
    ...
    (routesImports ? `${routesImports}\n` : `// (no modules declare routesEntry)\n`);
  ensureOutputDir(allRoutesGeneratedPath);
  fs.writeFileSync(allRoutesGeneratedPath, allRoutesGeneratedBody, 'utf8');

  // Generate all-portals.generated.ts — side-effect imports for portal page registrations
  const allPortalsGeneratedPath = path.join(rootDir, 'lib', 'portals', 'all-portals.generated.ts');
  const portalInitImports = resolvedModules
    .filter((mod) => mod.portalInitImportPath)
    .map((mod) => `import '${mod.portalInitImportPath}';`)
    .join('\n');
  const allPortalsGeneratedBody =
    `// AUTO-GENERATED by pnpm modules:prepare — do not edit manually\n` +
    `// Declare portalInit in module.json to register a module's portal pages here.\n` +
    (portalInitImports ? `${portalInitImports}\n` : `// (no modules declare portalInit)\n`);
  ensureOutputDir(allPortalsGeneratedPath);
  fs.writeFileSync(allPortalsGeneratedPath, allPortalsGeneratedBody, 'utf8');
  ...
  return {
    ...
    strictCompatibility,
    outputPath,
    allRoutesGeneratedPath,
    allPortalsGeneratedPath,
    warnings,
    configErrors,
    compatibilityErrors,
    resolvedModules
