...
type ChartTooltipFormatter = (
  value: number | string,
  name: string,
  payloadItem: ChartTooltipPayload,
  index: number,
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        data-chart={chartId}
        className={cn(
          "[&_.recharts-cartesian-axis-tick_text]:fill-muted-foreground [&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border/70 [&_.recharts-curve.recharts-tooltip-cursor]:stroke-border [&_.recharts-dot[stroke='#fff']]:stroke-transparent [&_.recharts-layer]:outline-none [&_.recharts-polar-grid_[stroke='#ccc']]:stroke-border [&_.recharts-radial-bar-background-sector]:fill-muted [&_.recharts-reference-line_[stroke='#ccc']]:stroke-border flex aspect-video justify-center text-xs",
          className
        )}
        {...props}
      >
...
function ChartTooltipContent({
  ...
  return (
    <div
      ...
      className={cn(
        "bg-background/95 grid min-w-[8rem] items-start gap-1.5 rounded-lg border border-border/60 px-2.5 py-1.5 text-xs shadow-xl",
        className
      )}
    >
...
function getPayloadConfigFromPayload(
  ...
  const payloadRecord = payloadItem as Record<string, unknown>;
